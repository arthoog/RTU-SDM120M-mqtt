...
class Dictionary {
  ...
  public:
    ...
    void inline operator = (Dictionary& dict) {
      destroy();
      merge(dict);
    }
    ...
    void inline printDictionary() {
      Serial.printf("\nDictionary::printDictionary:\n");
      printDictionary(iRoot);
      Serial.println();
    };
    void inline printArray() {
      Q->printArray();
    };
#endif
...
inline Dictionary::Dictionary(size_t init_size) {
  iRoot = NULL;
...
// ===== INSERTS =====================================================
int8_t inline Dictionary::insert(String keystr, String valstr) {
  return insert(keystr.c_str(), valstr.c_str());
}

int8_t inline Dictionary::insert(const char* keystr, const char* valstr) {
...
String inline Dictionary::search(String keystr) {
    return search(keystr.c_str());
}

String inline Dictionary::search(const char* keystr) {
    iKeyLen = strnlen(keystr, _DICT_KEYLEN + 1);
#ifdef _DICT_COMPRESS
    int8_t rc;
#endif
...
String inline Dictionary::key(size_t i) {
  if (Q) {
    node* p = (*Q)[i];
    if (p) {
#ifdef _DICT_COMPRESS
...
String inline Dictionary::value(size_t i) {
    if (Q) {
        node* p = (*Q)[i];
        if (p) {
#ifdef _LIBDEBUG_
...
// ==== DELETES =====================================================
void inline Dictionary::destroy() {
    destroy_tree(iRoot);
    iRoot = NULL;
    delete Q;
...
int8_t inline Dictionary::remove(String keystr) {
    return remove(keystr.c_str());
}

int8_t inline Dictionary::remove(const char* keystr) {
#ifdef _LIBDEBUG_
...
size_t inline Dictionary::jsize() {
    size_t ct = count();
...
size_t inline Dictionary::esize() {
    size_t ct = count();
    size_t sz = 0;
...
int8_t inline Dictionary::jload(String json, int aNum) {
...
int8_t inline Dictionary::merge(Dictionary& dict) {
...
bool inline Dictionary::operator () (String keystr) {
...
bool inline Dictionary::operator == (Dictionary& b) {
...
// ==== PRIVATE METHODS ====================================================
// ==== INSERTS ============================================================
int8_t inline Dictionary::insert(uintNN_t key, const char* keystr, _DICT_KEY_TYPE keylen, const char* valstr, _DICT_VAL_TYPE vallen, node* leaf) {
    if (key < leaf->key()) {
        if (leaf->left != NULL)
...
// ==== SEARCH ===========================================================================
inline node* Dictionary::search(uintNN_t key, node* leaf, const char* keystr, _DICT_KEY_TYPE keylen) {
    if (leaf != NULL) {
        if ( key == leaf->key() ) {
            int cmpres = keylen != leaf->ksize ? keylen - leaf->ksize : memcmp(leaf->keybuf, keystr, keylen);
#ifdef _LIBDEBUG_
...
// ==== DELETES ==========================================================================
void inline Dictionary::destroy_tree(node* leaf) {
  if (leaf != NULL) {
...
inline node* Dictionary::deleteNode(node* root, uintNN_t key, const char* keystr, _DICT_KEY_TYPE keylen) {
...
inline node* Dictionary::minValueNode(node* n) {
  node* current = n;
...
uintNN_t inline Dictionary::crc(const void* data, size_t n_bytes) {
    uintNN_t    a = 0;
...
// ==== COMPRESS METHODS =============================================
int8_t inline Dictionary::compressKey(const char* aStr) {
...
int8_t inline Dictionary::compressValue(const char* aStr) {
...
void inline Dictionary::decompressKey(const char* aBuf, _DICT_KEY_TYPE aLen) {
...
void inline Dictionary::decompressValue(const char* aBuf, _DICT_VAL_TYPE aLen) {
...
// ==== DEBUG METHODS ===================================================
#ifdef _LIBDEBUG_
void inline Dictionary::printDictionary(node* root) {
  if (root != NULL)
  {
...
void inline Dictionary::printNode(node* root) {
  if (root != NULL) {
