...
int8_t inline node::create(const char* aKey, _DICT_KEY_TYPE aKeySize, const char* aVal, _DICT_VAL_TYPE aValSize, node* aLeft, node* aRight) {
...
int8_t inline node::updateValue(const char* aVal, _DICT_VAL_TYPE aValSize) {
...
int8_t inline node::updateKey(const char* aKey, _DICT_KEY_TYPE aKeySize) {
...
// init the queue (constructor).
inline NodeArray::NodeArray(size_t init_size) {
...
// resize the size of the queue.
inline int8_t NodeArray::resize(const size_t s) {
  // defensive issue.
...
// add an item to the queue.
inline int8_t NodeArray::append(const node* i) {
  // check if the queue is full.
  if (isFull()) {
...
// remove an item from the queue.
void inline NodeArray::remove(const node* i) {
  // check if the queue is empty.
...
// check if the queue is empty.
bool inline NodeArray::isEmpty() const {
  return items == 0;
}

// check if the queue is full.
bool inline NodeArray::isFull() const {
  return items == size;
}

// get the number of items in the queue.
size_t inline NodeArray::count() const {
  return items;
}
} // namespace NodeArray
#endif // _NODEARRAY_H
